...
extension String {
    ...
    func markdown() -> AttributedString {
        ...
        var assets = MarkdownAsset()
        let atributes = assets.attributedString(from: document)
        return AttributedString(atributes)
    }
    
    func markdownAttribute() -> NSAttributedString {
        let document = Document(parsing: self)
        var assets = MarkdownAsset()
        return assets.attributedString(from: document)
    }
}

public struct MarkdownAsset: MarkupVisitor {
